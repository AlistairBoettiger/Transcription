...
function [ml1,ml2,m1b,m2b] = SeriesDecomp(G)  
...
%     iGb2 = inv(G{k}(2:end-1,2:end-1));
%     iG12 = iGb2^2;
%     iG22 = iGb2^3; 
%     
%  nx = [0,x{k}]'; % nx(nx==0) = 1; %denominator
%  inx = (nx~=0); inx(1)=0; inx(end) = 0; 
%   
%  mx = 1-[0,x{k}]';
%  imx = (mx~=0); imx(1)=0; imx(end) = 0; 
%  
%  i2x = G{k}(end,:)'~=0; % don't add terms where the numerator is zero (even if the denom is also zero)    
%  icnx = i2x&inx; % combine excluded terms
%  icnxs = icnx(2:end-1); % 
%   icmx = i2x&imx; % combine excluded terms
%  icmxs = icmx(2:end-1); %
...
%  save test;
%  if k~=1
%      ET_ccw(k) = -( G{k}(end,icnx) ./ (xs2(icnx)*G{k}(end,end)) ) * iG12(icnxs,icnxs)*(G{k}(icnx,end)) ; 
%      ET2_ccw(k) = 2*( G{k}(end,icnx) ./ (xs2(icnx)*G{k}(end,end)) ) *  iG22(icnxs,icnxs)*(G{k}(icnx,end)) ; 
%      ET_b(k) =  -( G{k}(end,icmx) ./ ((1-xs2(icmx))*G{k}(end,end)) ) *iG12(icmxs,icmxs)*(G{k}(icmx,1)) ; 
%      ET2_b(k) =  2*( G{k}(end,icmx) ./ ((1-xs2(icmx))*G{k}(end,end)) ) *iG22(icmxs,icmxs)*(G{k}(icmx,1)) ; 
%  end
 ...
 if k>1
   ...
   % M2(k,k-1) = P(k-1,k)*(ET_b(k-1) + ES(k-1));
   %  S2(k,k-1) = P(k-1,k)*(ET2_b(k-1)+ES2(k-1)+2*ET_b(k-1)*ES(k-1));
