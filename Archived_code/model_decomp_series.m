...
% This is incorrect.  All computations should be done in terms of G**
% % compute component Laplace transforms for forward jumps
% for k=1:n % index 
%     f = length(G{k+1}); 
%     Gi = inv(lambda*eye(f) - G{k+1});
%     expT(k) = lambda*Gi(1,f);
%     expS(k) = -G{k+1}(1,1)/(lambda - G{k+1}(1,1)); 
%     phi(k) = expT(k) + expS(k); % Laplace transform of tau_{k,k+1}
%     phip = diff(phi(k),lambda);
%     % now we can differentiate laplace transforms to get moments
%     M(k,k+1) = -subs(phip,lambda,0); % mean of tau_{k,k+1}
%     S(k,k+1) =  subs(diff(phip,lambda),lambda,0); % moment2 of tau_{k,k+1}
% end
